...
import { realLiveLoggedOutTest as test } from '../../../../../support/scenario-fixtures';

// Every scenario depends on the same unattended real-YouTube worker session.
// If that shared page cannot start, continuing would only repeat the same
// external setup failure in a fresh worker for every remaining scenario.
test.describe.configure({ mode: 'serial' });

test(
  'logged-out live: extension attaches and current tab action reports connected status',
  attachScenario
);
...
test(
  'logged-out live: replaced translations toggle from the inline icon',
  mockedReplacedTranslationToggleScenario
);
test(
  'logged-out live: focus panel opens from an author and follows their messages',
