...
import { expect, realYouTubeLoggedOutTest as test } from '../../support/browser-fixtures';
...
test('youtube-real performance: hybrid real-DOM busy stream stays responsive', async ({
  realLiveLoggedOutSession
}, testInfo) => {
  const { context, page } = realLiveLoggedOutSession;
      ...
      await withExtensionStorageValues(context, 'local', {
        ...
        try {
          ...
          const report = createPerformanceReport(
            'youtube-real hybrid real-DOM busy stream with translation and watched keywords',
            [
          ...
          await writePerformanceReport(testInfo, 'youtube-real-hybrid-real-dom', report);
