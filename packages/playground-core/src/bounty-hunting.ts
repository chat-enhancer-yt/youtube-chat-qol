const INVISIBLE_BOUNTY_TEXT_PATTERN = /[\u200B\u2060\uFEFF]/g;
...
export function countBountyHuntingTextEmojis(value: unknown): number {
  return Array.from(normalizeBountyHuntingText(value).matchAll(/\p{Extended_Pictographic}/gu)).length;
}

export function isBountyHuntingAllCapsMessage(value: unknown, minLetters = 4): boolean {
  const letters = Array.from(normalizeBountyHuntingText(value).matchAll(/\p{L}/gu)).map((match) => match[0]);
...
function normalizeBountyHuntingText(value: unknown): string {
  return String(value || '').replace(INVISIBLE_BOUNTY_TEXT_PATTERN, '').replace(/\s+/g, ' ').trim();
}
