...
const SHARED_YOUTUBE_SELECTOR_HINTS = [
  ...
  {
    pattern:
      /(?:yt-live-chat-item-list-renderer\s+#item-scroller|yt-live-chat-renderer\s+#item-scroller|#item-scroller)/,
  ...
  {
    pattern:
      /(?:#send-button|yt-button-renderer#send-button|yt-icon-button#send-button|button\[(?:aria-label|title)=["']Send["']\])/,
  ...
  rules: {
    ...
    'require-jsx-dom-el-type': {
      meta: {
        ...
        messages: {
          missingType:
            'Pass an explicit result type to el(), for example el<HTMLDivElement>(...) or el<Comment>(...).'
        },
        schema: []
      },
      create(context) {
    ...
    'no-direct-jsx-factory': {
      ...
      create(context) {
        ...
        return {
          ...
          CallExpression(node) {
            const callee = node.callee;
            const callsNamedFactory =
              callee?.type === 'Identifier' && jsxFactoryNames.has(callee.name);
            const callsNamespaceFactory =
    ...
    'require-global-listener-signal': {
      meta: {
        ...
        docs: {
          description:
            'Require AbortSignal-backed cleanup for document/window listeners in feature code.'
        },
        messages: {
          missingSignal:
    ...
    'no-hardcoded-visible-ui-literals': {
      meta: {
        ...
        docs: {
          description:
            'Warn when visible extension UI text is hardcoded instead of coming from localization.'
        },
        messages: {
          useLocalizedText:
...
function getStaticString(node) {
  ...
  if (unwrapped.type === 'Literal')
    return typeof unwrapped.value === 'string' ? unwrapped.value : '';
