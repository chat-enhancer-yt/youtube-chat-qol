...
const DEFAULT_UNIT_REPORT_JSON = path.join(repoRoot, 'test-results', 'unit', 'vitest-report.json');
...
function getUnitReportPath() {
  if (process.env.YTCQ_UNIT_REPORT_JSON) {
    return path.isAbsolute(process.env.YTCQ_UNIT_REPORT_JSON)
      ? process.env.YTCQ_UNIT_REPORT_JSON
      : path.join(repoRoot, process.env.YTCQ_UNIT_REPORT_JSON);
  }
