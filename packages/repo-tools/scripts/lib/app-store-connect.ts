...
type AppStoreConnectErrorOptions = {
  payload?: unknown;
  status?: number | null;
  url?: string | null;
};
...
export class AppStoreConnectError extends Error {
  ...
  constructor(
    message: string,
    { payload = null, status = null, url = null }: AppStoreConnectErrorOptions = {}
  ) {
    super(message);
...
export async function appStoreConnectFetch(config, pathOrUrl, options: any = {}) {
