...
type WalkthroughVideos = Record<string, string>;

if (process.argv[1] && path.resolve(process.argv[1]) === scriptPath) {
  await publishWalkthroughVideos(readOptions(process.argv.slice(2)));
}

export async function publishWalkthroughVideos(options: any = {}) {
...
function validateManifestEntries(videos: WalkthroughVideos, supportedLocales: string[]) {
  const supportedLocaleSet = new Set(supportedLocales);
...
function validateCompleteManifest(videos: WalkthroughVideos, supportedLocales: string[]) {
...
function readRequestedLocales(value: string | undefined, supportedLocales: string[]) {
...
export async function verifyLiveWalkthroughReferences(
  siteBaseUrl: string,
  currentVideos: WalkthroughVideos,
  locales: string[]
) {
