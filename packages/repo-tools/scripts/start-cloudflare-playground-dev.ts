...
const WRANGLER_CONFIG_PATH = path.join(REPO_ROOT, 'apps', 'playground', 'wrangler.toml');
...
const wrangler = spawnSync(process.execPath, [
  ...
  env: {
    ...
    WRANGLER_LOG_PATH: process.env.WRANGLER_LOG_PATH || path.join(REPO_ROOT, '.wrangler', 'logs')
  },
  cwd: REPO_ROOT,
  stdio: 'inherit'
});
...
function runDocker(args, options: any = {}) {
