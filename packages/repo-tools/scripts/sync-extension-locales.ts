...
const localesSourceDir = path.join(root, 'apps', 'extension', 'src', 'shared', 'locales');
const defaultOutputDir = path.join(root, 'dist', 'extension-chrome', '_locales');
const webExtensionLocaleAliases: Record<string, string[]> = {
...
type LocaleCatalog = {
  extension?: Record<string, ExtensionMessage>;
};
...
async function readLocaleCatalogs(): Promise<Record<string, LocaleCatalog>> {
...
function createExtensionMessages(
  defaultMessages: Record<string, ExtensionMessage>,
  localeMessages: Record<string, ExtensionMessage>
) {
...
function getWebExtensionLocales(locale: string) {
  return webExtensionLocaleAliases[locale] || [locale];
}

function toWebExtensionMessage(message: ExtensionMessage) {
