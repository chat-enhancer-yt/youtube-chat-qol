 ...
 *
 * Run through the repo's `publish:safari` or `build:safari:archive` command after packaging Safari.
 */
...
} from './lib/safari-export-options.ts';
...
function printUsage() {
  console.log(`
Usage:
  npm run safari:distribute -w @chatenhancer/repo-tools -- --archive
  npm run safari:distribute -w @chatenhancer/repo-tools -- --upload
...
function run(command, commandArgs, options: any = {}) {
...
function runCapture(command, commandArgs, options: any = {}) {
  const result = spawnSync(command, commandArgs, {
    cwd: root,
    encoding: 'utf8',
    ...options
